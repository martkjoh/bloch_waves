...
def get_H(N, L, V_period):
    ...
    H[1::3] = 2/dx**2 + V_period(x%1)
    H[0::3] = H[2::3] = -1/dx**2 * np.ones_like(x)
...
def V_sin(freqs):
    V0 = -5
    return lambda x : np.sum([V0*sin(2*pi*x*i) for i in range(freqs+1)], axis=0)


def main():
    L = 50 # Number of periods in lattice
    m = 30 # Number of points in a period
    N = m*L
    numVec = 160
    n = 4
    Vs = [V_sin(i) for i in range(n)]
    for i in range(n):
